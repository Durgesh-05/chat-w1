...
export const RoomCard = ({ users, roomId, currentUserId }: { users: User[], roomId: string, currentUserId: string | undefined }) => {
        ...
        {otherUsers.length === 1 ? (
          <>
            <Avatar className="w-12 h-12">
              <AvatarImage src={otherUsers[0].profileImage} alt={otherUsers[0].firstName} />
              <AvatarFallback>{otherUsers[0].firstName[0]}</AvatarFallback>
            </Avatar>
            <div className="flex flex-col justify-center items-start">
              <div className="text-gray-900 font-semibold">{otherUsers[0].firstName}</div>
          ...
          <div className="flex -space-x-4">
            {otherUsers.slice(0, 3).map((user, index) => (
              <Avatar key={user.id} className={`w-10 h-10 border-2 border-white ${index > 0 ? '-ml-2' : ''}`}>
